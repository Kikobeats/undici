...
const { forbiddenMethods } = require('../fetch/constants')
const { safelyExtractBody } = require('../fetch/body')
const { Fetch, finalizeAndReportTiming, fetching } = require('../fetch/index')
const { HeadersList } = require('../fetch/headers')
const { makeNetworkError } = require('../fetch/response')
...
class XMLHttpRequest extends XMLHttpRequestUpload {
  ...
  get response () {
    ...
    if (this[kResponseType] === 'arraybuffer') {
      ...
      //    MIME type for this.
      // TODO: mime type
      this[kResponseObject] = new Blob(this[kReceivedBytes])
  ...
  get readyState () {
    return XMLHttpRequestReadyState[this[kState].toUpperCase()]
  }
